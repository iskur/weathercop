"""VG configuration file to be used with KLL data and WeatherCop."""
...
dists = {
    ...
    "rh":              "empirical",
    # "abs_hum":         "empirical",
    "abs_hum":         distributions.truncnorm,
    # "abs_hum":         (distributions.Rain, distributions.norm),
    # "R":               distributions.expon,
    # "R":               distributions.lognormal,
    "R":               (distributions.RainMix, distributions.kumaraswamy),
    # "R":               (distributions.Rain, distributions.weibull),
    # "R":               (distributions.Rain, distributions.lognormal),
    # "R":               "empirical",
    ...
    # "theta":           stats.distributions.gumbel_r,
...
dists_kwds = {
    ...
    "theta": dict(doy_width=15, fft_order=6, tabulate_cdf=True),
    # "abs_hum": dict(doy_width=10),
...
def array_gen(scalar):
    ...
    return lambda tt: np.full((len(np.atleast_1d(tt)), len(scalar)), scalar)
...
def max_qsw(doys, **kwds):
    ...
    for i, doy in enumerate(doys):
        ...
        qsw_2d[:, i] = meteox2y_cy.pot_s_rad(date,
                                             longt=longitude,
                                             lat=latitude,
                                             in_format="%j %H", **kwds)
...
def max_sunshine_hours(doys):
    ...
    sun_hours = meteox2y.sunshine_hours(dates,
                                        longitude=longitude,
                                        latitude=latitude,
...
par_known.update({
    ...
    "rh":          {"lc": array_gen(0),
                    # "uc": array_gen(hpd * 1.01),  # shajar does not like that
                    "uc": array_gen(hpd),
                    },
    "abs_hum":     {"lc": array_gen(0),
                    "uc": array_gen(100 * 24)},
    "U":           {"l": array_gen(0.)},
...
# def abs_hum2rel(times_, data, var_names):
#     try:
#         abs_hum_i = var_names.index("abs_hum")
#     except ValueError:
#         warnings.warn("No 'abs_hum' in data.")
#         return times_, data, var_names
#     abs_hum = data[abs_hum_i]
#     theta = data[var_names.index("theta")]
#     rh = meteox2y.abs_hum2rel(abs_hum, theta)
#     data[abs_hum_i] = rh
#     var_names[abs_hum_i] = "rh"
#     return times_, data, var_names


# conversions += [abs_hum2rel]
