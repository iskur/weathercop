...
try:
    ...
    warnings.warn("Could not import theano.", ImportWarning)
    THEANO = False
...
class MetaCop(ABCMeta):
    ...
    def inverse_conditional_cdf(cls, conditioning):
        ...
        try:
            ...
            warnings.warn(f"Could not compile inv.-conditional for {cls.name}", UserWarning)
            mark_failed(key)
            return
        return ufunc
...
class Copulae(metaclass=MetaCop):
    ...
    def _inverse_conditional(
        ...
        *theta,
        ...
        ranks1 = np.asarray(ranks1, dtype=np.float64).ravel()
        quantiles_array = np.asarray(quantiles_array, dtype=np.float64).ravel()
            ...
            if callable(attr):
                ...
                def proxied(*args, **kwds):
                    ...
                    if "theta" in pars:
                        try:
                            theta = np.full_like(
                                args[0], self.theta[0], dtype=np.float64
                            )
                        except IndexError:
                            theta = self.theta
