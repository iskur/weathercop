...
class Test(npt.TestCase):
    ...
    def test_serialize(self):
        for vine in (self.cvine, self.rvine):
    ...
    def test_cquantiles(self):
        ...
        try:
            ...
            print("Obs\n",
                  pd.DataFrame(np.corrcoef(self.data_ranks).round(3),
                               index=self.varnames,
                               columns=self.varnames))
            print("Sim\n", pd.DataFrame(np.corrcoef(sim).round(3),
                                        index=self.varnames,
                                        columns=self.varnames))
            fig, axs = plt.subplots(nrows=self.K, sharex=True)
            for i, ax in enumerate(axs):
    ...
    def test_seasonal_ph(self):
        ...
        for _ in range(n_realizations):
            simt0, sim0 = met_vg.simulate( #  theta_incr=0.,
                                          sim_func=vg_ph)
            means_norm0 += [met_vg.sim.mean(axis=1)]
            simt1, sim1 = met_vg.simulate(theta_incr=theta_incr,
                                          sim_func=vg_ph)
        ...
        means_df = pd.DataFrame(data_dict,
                                index=met_vg.var_names)
        print(means_df)
        npt.assert_almost_equal(met_vg.data_trans.mean(axis=1),
                                np.mean(means_norm0, axis=0), decimal=2)
        means0[prim_i] += theta_incr
        means_obs = met_vg.data_raw.mean(axis=1) / 24
        means_obs[prim_i] += theta_incr
        npt.assert_almost_equal(means_obs[prim_i], means1[prim_i],
                                decimal=2)
        npt.assert_almost_equal(means0[prim_i], means1[prim_i],
                                decimal=2)
