...
class Test(npt.TestCase):
    ...
    def test_inverse_conditional(self):
        ...
        theta = np.full_like(uu, .75)
        time0 = time.clock()
        vv_act = np.empty_like(uu)
        normal_conditional.norm_inv_cdf_given_u(uu, qq, theta, vv_act)
        time_cy = time.clock()
        vv_exp = cop.gaussian.inv_cdf_given_u_(uu, qq, theta)
        time_py = (time.clock() - time_cy)
        time_cy -= time0
    ...
    def test_conditional(self):
        ...
        time_cy = time.clock()
        qq_exp = cop.gaussian.cdf_given_u_(uu, vv, theta)
        time_py = (time.clock() - time_cy)
        time_cy -= time0
        print("Time python: ", time_py * 1e6)
        print("Time cython: ", time_cy * 1e6)
        try:
            ...
            ax.scatter(qq_exp, qq_act, s=4,
                       facecolor=(0, 0, 0, .8),
                       edgecolor=(0, 0, 0, 0))
            ax.set_title("2 Implementations of the conditional gaussian copula cdf (one has a bug!)")
            plt.show()
            raise
